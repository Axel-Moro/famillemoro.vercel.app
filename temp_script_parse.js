...
async function loadLocalData() {
  try {
    ...
    try {
      const rapportsRes = await sb.from('rapports').select('*');
      if (rapportsRes.data) DB.rapports = rapportsRes.data;
    } catch (rapportsError) {
      console.warn('Supabase rapports load failed (table missing ?)', rapportsError);
    }
...
async function insertUser(user) {
  ...
  const { data, error } = await sb.from('membres').insert([payload]);
...
async function saveTotalWithdrawal() {
  ...
  try {
    var amount = parseCurrencyAmount(document.getElementById('totalWithdrawAmount').value);
    ...
    var chosenYear = parseCurrencyAmount(document.getElementById('totalWithdrawYear').value);
...
function saveWithdrawal() {
  ...
  var amount = parseCurrencyAmount(document.getElementById('withdrawAmount').value);
